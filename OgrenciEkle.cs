...
namespace OkulOtomasyonSitemi
{
    ...
    {
        int flagForOld = 0;     // Yaş doğrualaması için
        ...
        {
            MessageBox.Show("Boşlukları Doğru Şekilde Doldurarak Öğrenci Kayıt İşleminizi Gerçekleştirebilirsiniz.", "İnfo", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int okulNoKontrol = okulNoDogrula(textBoxOkulno.Text);


            if (textBoxOkulno.Text != "" && textBoxTC.Text != "" && textBoxName.Text != "" &&
                ...
                dateTimePicker1.Value != DateTime.Now && (radioButton1.Checked == true || radioButton2.Checked == true || radioButton3.Checked == true))
            {
                if (okulNoKontrol != 1)
                {
                    if (tcKontrol == 1)
                    {
                        if( flagForOld != 1 )
                        {
                            ...
                            OleDbCommand cmd = new OleDbCommand("INSERT INTO ogrenciler (okulno, tc,  ad, soyad, ulke, sehir, telefon, eposta, bolum, dogumtarihi, cinsiyet) VALUES (@okulno, @tc , @ad, @soyad, @ulke, @sehir, @telefon, @eposta, @bolum, @dogumtarihi, @cinsiyet)", conn);

                            cmd.Parameters.AddWithValue("@okulno", textBoxOkulno.Text);
                            ...
                            try
                            {
                                cmd.ExecuteNonQuery();
                                conn.Close();
                                MessageBox.Show("Yeni Öğrenci Bilgileri Sisteme Eklendi!", "İşlem Başarılı.");
                                textBoxOkulno.Clear();
                                textBoxTC.Clear();
                                textBoxName.Clear();
                            ...
                            {
                                MessageBox.Show("BÖYLE BİR ÖĞRENCİ SİSTEMDE KAYITLI!!! ", "Başarısız!");
                                conn.Close();
                            }
                            finally
                            {
                ...
                {
                    MessageBox.Show("Bu Okul Numarasına Sahip Öğrenci Sİstemde Kayıtlıdır!", "Başarısız!");
                }
            } else
            {
        ...
        private int okulNoDogrula(string okulno)
        {
            int flag = 0;

            conn.Open();
            OleDbCommand cmd = new OleDbCommand("Select * from ogrenciler where okulno = '" + textBoxOkulno.Text + "'", conn);
            OleDbDataReader read = cmd.ExecuteReader();
            if (read.Read() == true)
            {
                if (textBoxOkulno.Text == read["okulno"].ToString())
                {
                    flag = 1;
                }
            }
